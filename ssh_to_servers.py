...
def get_template(df):
    ...
    return text_xml_template


with open('grid.sh', 'r') as file:
    grid_script = file.read()
...
check_status = []
errors = []
nodes = list(df[~df['node'].isna()]['node'].unique())

for node in nodes:
    staging_df = df[df['node'] == node]
    for index, row in staging_df.iterrows():
        ...
        try:
            ...
            if type == 'Grid':
                ...
                text_xml_template = get_template(staging_df)
                ...
                dealer_crawler = dealer_crawler.replace('104.238.228.193', staging_df[staging_df['type'] == 'Load Balancer']['IP'].iloc[0])
